...
import { learningSafetyApi, type AuthorshipSummary } from '@/lib/api'
...
export function EnemWritingStudioPage() {
  ...
  const [authorship, setAuthorship] = useState<AuthorshipSummary | null>(null)
  const wordCount = useMemo(() => text.trim() ? text.trim().split(/\s+/).length : 0, [text])

  const loadAuthorship = async (workId:string) => {
    try { const result = await learningSafetyApi.authorship(workId); setAuthorship(result.summary) }
    catch { setAuthorship(null) }
  }
  useEffect(()=>{ if(active?.id) void loadAuthorship(active.id); else setAuthorship(null) },[active?.id, active?.versions.length])

  const start = () => { const [area, theme] = starterThemes[themeIndex]; const id = createProject(theme, area); setActiveProject(id); setText(''); setSyncState('idle'); setAuthorship(null) }
  const save = async () => {
    if (!active || !text.trim()) return
    const eventType = active.versions.length === 0 ? 'student_original' : 'student_revision'
    saveVersion(active.id, text)
    void learningSafetyApi.addAuthorshipEvent({workId:active.id,type:eventType,content:text,metadata:{theme:active.theme,versionNumber:active.versions.length+1,source:'enem_writing_studio'}}).then(()=>loadAuthorship(active.id)).catch(()=>undefined)
  ...
  return <main className="ms-screen space-y-4 py-4"><section><button onClick={()=>setActiveProject(null)} className="text-xs font-black text-violet-600">← Meus textos</button><p className="mt-3 text-xs font-black uppercase tracking-wider text-violet-600">{active.area} · versão {active.versions.length + 1}</p><h1 className="mt-1 text-xl font-black leading-7">{active.theme}</h1><p className={`mt-2 text-[11px] font-bold ${syncState==='error'?'text-amber-600':syncState==='synced'?'text-emerald-600':'text-slate-400'}`}>{syncLabel}</p></section><section className="rounded-3xl bg-white p-4 shadow-sm"><textarea value={text} onChange={(e)=>setText(e.target.value)} placeholder="Comece pelas suas ideias. Não precisa ficar perfeito na primeira versão..." className="min-h-[390px] w-full resize-none bg-transparent text-[15px] leading-7 outline-none placeholder:text-slate-300"/><div className="flex items-center justify-between border-t border-slate-100 pt-3 text-xs font-bold text-slate-400"><span>{wordCount} palavras</span><span>{active.versions.length} versões salvas</span></div></section><div className="grid grid-cols-2 gap-3"><button onClick={save} disabled={!text.trim() || syncState==='syncing'} className="rounded-2xl bg-slate-950 py-4 text-sm font-black text-white disabled:opacity-40">{syncState==='syncing'?'Salvando…':'Salvar versão'}</button><button onClick={()=>navigate('/enem/redacao/dna')} className="rounded-2xl bg-violet-100 py-4 text-sm font-black text-violet-800">Ver meu DNA</button></div>
  {authorship&&authorship.events>0&&<section className="rounded-3xl bg-emerald-50 p-5"><div className="flex items-center gap-2 text-emerald-700"><Fingerprint className="h-5 w-5"/><p className="text-xs font-black uppercase tracking-wider">Mapa de autoria</p></div><h2 className="mt-2 text-lg font-black text-emerald-950">Seu processo fica visível — sua voz continua sendo sua.</h2><div className="mt-4 grid grid-cols-3 gap-2 text-center"><AuthMetric label="Estudante" value={authorship.studentAuthoredShare}/><AuthMetric label="Apoio IA" value={authorship.aiAssistanceShare}/><AuthMetric label="Revisão humana" value={authorship.humanReviewShare}/></div><div className="mt-4 flex items-start gap-2 rounded-2xl bg-white/70 p-3 text-xs font-bold leading-5 text-emerald-950/70"><ShieldCheck className="mt-0.5 h-4 w-4 shrink-0"/>Esse mapa registra o processo documentado. Não é detector de plágio nem afirma autoria forense.</div></section>}
  {active.versions.length >= 2 && <button onClick={()=>navigate('/enem/redacao/comparar')} className="w-full rounded-2xl bg-emerald-600 py-4 text-sm font-black text-white">Comparar minhas duas últimas versões ↗</button>}{active.versions.length>0 && <section className="rounded-3xl bg-indigo-50 p-5"><p className="text-xs font-black uppercase tracking-wider text-indigo-600">Histórico vivo</p><h2 className="mt-1 font-black">Você não perde versões anteriores.</h2><div className="mt-3 space-y-2">{active.versions.slice().reverse().slice(0,3).map((v,i)=><button key={v.id} onClick={()=>setText(v.text)} className="flex w-full items-center justify-between rounded-2xl bg-white p-3 text-left"><span className="text-sm font-black">Versão {active.versions.length-i}</span><span className="text-xs font-bold text-slate-400">{v.wordCount} palavras</span></button>)}</div></section>}{active.versions.length===1 && <section className="rounded-3xl bg-amber-50 p-5"><p className="text-xs font-black uppercase tracking-wider text-amber-700">Próximo desbloqueio</p><h2 className="mt-1 font-black text-amber-950">Salve uma segunda versão.</h2><p className="mt-2 text-sm leading-6 text-amber-900/70">A partir daí o MindSteps começa a comparar como sua escrita mudou, em vez de olhar apenas para o texto final.</p></section>}</main>
}

function AuthMetric({label,value}:{label:string;value:number}){return <div className="rounded-2xl bg-white p-3"><p className="text-2xl font-black text-emerald-950">{value}%</p><p className="mt-1 text-[10px] font-black uppercase tracking-wider text-emerald-700">{label}</p></div>}
