...
import { ArrowRight, Brain, BrainCircuit, Camera, ChevronRight, Compass, Dna, FileText, Flame, GraduationCap, Link2, Loader2, MessageCircle, RefreshCw, ShieldCheck, Shuffle, Sparkles, Target } from 'lucide-react'
...
const actions = [
  ...
  { title: 'Estudar pra prova', subtitle: 'Monte um plano', icon: GraduationCap, path: '/enem', className: 'bg-violet-50 text-violet-700' },
  ...
  { title: 'Revisar rapidinho', subtitle: 'Poucos minutos', icon: RefreshCw, path: '/enem/erros', className: 'bg-amber-50 text-amber-700' },
]
...
export function TodayPage() {
  ...
  useEffect(() => {
    operationsApi.today().then(setOverview).catch((err) => setError(err instanceof Error ? err.message : 'Não foi possível carregar seu dia')).finally(() => setLoading(false))
  }, [])
  ...
  const orgCount = overview?.organizations?.filter(item => item.status === 'active').length || 0
  ...
  return <div className="ms-screen space-y-5 py-4">
    ...
    <section><button onClick={()=>navigate('/independencia')} className="ms-pressable w-full rounded-[30px] bg-gradient-to-br from-emerald-600 to-cyan-700 p-5 text-left text-white shadow-lg shadow-emerald-100"><div className="flex items-start gap-4"><span className="flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl bg-white/15"><Brain className="h-6 w-6"/></span><span className="min-w-0 flex-1"><span className="block text-xs font-black uppercase tracking-[.16em] text-emerald-100">Agora sem rodinhas</span><span className="mt-1 block text-lg font-black">Descubra o que você já consegue fazer sozinho.</span><span className="mt-2 block text-sm leading-5 text-white/75">Checks curtos, sem pistas, para medir autonomia — não só desempenho com ajuda.</span></span><ChevronRight className="mt-1 h-5 w-5 shrink-0 text-white/70"/></div></button></section>

    <section><button onClick={()=>navigate('/responsavel')} className="ms-card-soft ms-pressable flex w-full items-center gap-3 p-4 text-left"><span className="flex h-12 w-12 items-center justify-center rounded-2xl bg-indigo-50 text-indigo-700"><ShieldCheck className="h-6 w-6"/></span><span className="min-w-0 flex-1"><span className="block text-sm font-black text-slate-950">Entenda sua IA</span><span className="mt-0.5 block text-xs font-medium text-slate-500">IA & Cidadania, por que o MindSteps decidiu algo e seus controles de dados.</span></span><ChevronRight className="h-5 w-5 text-slate-300"/></button></section>

    {features.enemV2 && <section className="relative overflow-hidden rounded-[30px] border border-violet-100 bg-gradient-to-br from-violet-50 via-white to-indigo-50 p-5"><div className="absolute -right-10 -top-12 h-32 w-32 rounded-full bg-violet-100/70 blur-xl"/><div className="relative"><div className="flex items-start gap-4"><span className="flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl bg-violet-600 text-white"><GraduationCap className="h-6 w-6"/></span><div className="min-w-0 flex-1"><p className="text-xs font-black uppercase tracking-wider text-violet-600">Destaque · MindSteps ENEM</p><h3 className="mt-1 text-lg font-black text-slate-950">Seu plano, suas questões e sua escrita no mesmo mapa.</h3><p className="mt-1 text-sm leading-5 text-slate-500">Diagnóstico adaptativo, Meu Dia ENEM, redação, simulados estratégicos, semana e caderno de erros.</p></div></div><div className="mt-4 grid grid-cols-4 gap-2">{enemWriting.map(({title,icon:Icon,path,cls})=><button key={title} onClick={()=>navigate(path)} className="ms-pressable rounded-2xl bg-white p-2.5 text-center shadow-sm ring-1 ring-slate-100"><span className={`mx-auto flex h-9 w-9 items-center justify-center rounded-xl ${cls}`}><Icon className="h-4 w-4"/></span><span className="mt-1.5 block text-[10px] font-black text-slate-700">{title}</span></button>)}</div><button onClick={() => navigate('/enem')} className="mt-4 inline-flex items-center gap-1 text-sm font-black text-violet-700">Abrir meu ENEM <ChevronRight className="h-4 w-4"/></button></div></section>}

    <section><button onClick={()=>navigate('/vinculos')} className="ms-card-soft ms-pressable flex w-full items-center gap-3 p-4 text-left"><span className="flex h-12 w-12 items-center justify-center rounded-2xl bg-emerald-50 text-emerald-700"><Link2 className="h-6 w-6"/></span><span className="min-w-0 flex-1"><span className="block text-sm font-black text-slate-950">{orgCount ? `${orgCount} vínculo${orgCount>1?'s':''} ativo${orgCount>1?'s':''}` : 'Conectar escola, turma ou programa'}</span><span className="mt-0.5 block text-xs font-medium text-slate-500">Sua conta e seu histórico continuam sendo os mesmos.</span></span><ChevronRight className="h-5 w-5 text-slate-300"/></button></section>
