...
export function TodayPage() {
  ...
  return <div className="ms-screen space-y-5 py-4">
    ...
    <section className="relative overflow-hidden rounded-[28px] border border-violet-100 bg-gradient-to-br from-violet-50 via-white to-indigo-50 p-5"><div className="absolute -right-10 -top-12 h-32 w-32 rounded-full bg-violet-100/70 blur-xl"/><div className="relative flex items-start gap-4"><span className="flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl bg-violet-600 text-white"><GraduationCap className="h-6 w-6"/></span><div className="min-w-0 flex-1"><p className="text-xs font-black uppercase tracking-wider text-violet-600">Novo · MindSteps ENEM</p><h3 className="mt-1 text-lg font-black text-slate-950">Prepare-se sem virar refém de cursinho.</h3><p className="mt-1 text-sm leading-5 text-slate-500">Redação, questões, mapa de lacunas e treino diário adaptativo.</p><button onClick={() => navigate('/enem')} className="mt-4 inline-flex items-center gap-1 text-sm font-black text-violet-700">Explorar ENEM <ChevronRight className="h-4 w-4"/></button></div></div></section>
