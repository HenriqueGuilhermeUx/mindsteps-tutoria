...
import { analyzeWriting, scoreDelta, writingSkillLabels } from '@/lib/writingAnalysis'
...
export function WritingDNAPage() {
  ...
  const { projects, drillResults, setActiveProject } = useWritingStore()
  ...
  const effects = useMemo(() => drillResults.flatMap(drill => {
    const project = projects.find(p => p.id === drill.projectId); if (!project || !drill.sourceVersionId) return []
    const sourceIndex = project.versions.findIndex(v => v.id === drill.sourceVersionId); if (sourceIndex < 0 || sourceIndex >= project.versions.length - 1) return []
    const before = project.versions[sourceIndex], after = project.versions[sourceIndex + 1]
    return [{ drill, project, before, after, delta: scoreDelta(before.text, after.text, drill.skill) }]
  }), [drillResults, projects])
  ...
  return <main className="mx-auto w-full max-w-md px-4 pb-28 pt-2 text-slate-900">
    ...
    {!latestProjects.length ? <section className="mt-5 rounded-3xl bg-white p-5 shadow-sm"><h2 className="font-black">Seu DNA ainda está em branco.</h2><p className="mt-2 text-sm leading-6 text-slate-500">Salve sua primeira redação para começar a construir seu mapa.</p><button onClick={()=>navigate('/enem/redacao/escrever')} className="mt-4 w-full rounded-2xl bg-violet-600 py-3.5 text-sm font-black text-white">Escrever primeira versão</button></section> : <>
      ...
      <section className="mt-6 rounded-3xl bg-indigo-50 p-5"><div className="flex items-center gap-2 text-indigo-700"><Sparkles className="h-4 w-4"/><span className="text-xs font-black uppercase tracking-wider">Próxima intervenção</span></div><h2 className="mt-2 text-lg font-black">Treinar {writingSkillLabels[next.skill]}</h2><p className="mt-2 text-sm leading-6 text-indigo-900/70">O próximo treino deve atacar sua habilidade menos consolidada e depois verificar se ela melhora na nova versão.</p><button onClick={() => navigate(`/enem/redacao/treino/${next.skill==='argumentacao'?'argumento':next.skill==='clareza'||next.skill==='coesao'?'paragrafo':next.skill}`)} className="mt-4 flex w-full items-center justify-center gap-2 rounded-2xl bg-indigo-700 py-3.5 text-sm font-black text-white">Começar treino <ChevronRight className="h-4 w-4"/></button></section>

      <section className="mt-6"><div className="flex items-center gap-2"><TrendingUp className="h-5 w-5 text-emerald-600"/><h2 className="text-lg font-black">O que funciona para você</h2></div>{effects.length ? <><div className="mt-3 grid grid-cols-2 gap-3"><div className="rounded-3xl bg-white p-4 shadow-sm"><p className="text-2xl font-black text-emerald-600">{positiveRate}%</p><p className="mt-1 text-xs font-bold text-slate-500">dos treinos medidos tiveram sinal positivo na versão seguinte</p></div><div className="rounded-3xl bg-white p-4 shadow-sm"><p className="text-2xl font-black text-indigo-600">{effects.length}</p><p className="mt-1 text-xs font-bold text-slate-500">intervenções já comparáveis</p></div></div>{bestEffect && <div className="mt-3 rounded-3xl bg-emerald-50 p-5"><p className="text-xs font-black uppercase tracking-wider text-emerald-700">Maior resposta observada</p><p className="mt-2 text-lg font-black text-emerald-950">{writingSkillLabels[bestEffect.drill.skill]} {bestEffect.delta>=0?'+':''}{bestEffect.delta}</p><p className="mt-1 text-sm leading-6 text-emerald-900/70">Depois desse treino, o sinal da habilidade aumentou na versão seguinte de “{bestEffect.project.theme}”.</p></div>}</> : <div className="mt-3 rounded-3xl bg-white p-5 shadow-sm"><p className="text-sm font-black">Ainda precisamos de um ciclo completo.</p><p className="mt-2 text-sm leading-6 text-slate-500">Faça um treino recomendado, reescreva e salve uma nova versão. A partir daí o MindSteps começa a medir quais intervenções parecem funcionar melhor para você.</p></div>}</section>
