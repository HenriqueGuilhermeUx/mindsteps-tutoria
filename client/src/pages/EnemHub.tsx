...
import { BookOpenCheck, ChevronRight, Clock3, FileText, GraduationCap, Sparkles, Target, TimerReset } from 'lucide-react'
...
export function EnemHubPage() {
  ...
  return <main className="ms-screen space-y-5 py-4">
    ...
    <section className="grid grid-cols-2 gap-3">{tools.map(({title,subtitle,icon:Icon,path,tone}) => <button key={title} onClick={() => navigate(path)} className="ms-card-soft ms-pressable min-h-[148px] p-4 text-left"><span className={`flex h-11 w-11 items-center justify-center rounded-2xl ${tone}`}><Icon className="h-5 w-5" /></span><h2 className="mt-4 text-sm font-black text-slate-950">{title}</h2><p className="mt-1 text-xs leading-5 text-slate-500">{subtitle}</p></button>)}</section>
