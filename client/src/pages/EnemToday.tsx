...
export function EnemTodayPage(){
 const navigate=useNavigate(); const {projects}=useWritingStore(); const {profile,attempts,dailyCompleted,toggleDailyStep}=useEnemStore()
 const today=new Date().toISOString().slice(0,10)
 const focus=useMemo(()=>{
   const candidates:Array<{name:EnemArea;value:number;path:string;skill:string;source:'evidência'|'autopercepção'|'neutro'}>=[]
   ;(Object.keys(areaMeta) as Array<Exclude<EnemArea,'Redação'>>).forEach(name=>{
     const list=attempts.filter(a=>a.area===name).slice(-20)
     const hasEvidence=list.length>=2
     const fallback=profile ? profile.confidence[name]*20 : 50
     const value=hasEvidence?Math.round(30+(list.filter(a=>a.correct).length/list.length)*70):fallback
     candidates.push({name,value,...areaMeta[name],source:hasEvidence?'evidência':profile?'autopercepção':'neutro'})
   })
   const writing=projects.filter(p=>p.versions.length).slice(0,5)
   let writingValue=profile ? profile.confidence['Redação']*20 : 50
   let writingSource:'evidência'|'autopercepção'|'neutro'=profile?'autopercepção':'neutro'
