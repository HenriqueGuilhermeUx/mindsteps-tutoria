...
export interface TransferCheck{id:string;subject?:string|null;skill?:string|null;source:string;assisted_score?:number|null;independent_score?:number|null;help_level:number;attempt_before_help:boolean;completed:boolean;created_at:string;metadata?:Record<string,unknown>}
export interface IndependentChallenge{question:string;rubric:string[];expectedMinutes:number}
export interface IndependentEvaluation{score:number;feedback:string;evidence:string[];passed:boolean}
export const independentLearningApi={
 ...
 challenge:(data:{subject:string;skill?:string;context?:string})=>call<{challenge:IndependentChallenge}>('/api/learning-os/transfer-challenge',{method:'POST',body:data}),
 evaluate:(data:{question:string;answer:string;subject:string;skill?:string;rubric?:string[];assistedScore?:number;helpLevel?:number;attemptBeforeHelp?:boolean;source?:string})=>call<{evaluation:IndependentEvaluation;check:TransferCheck;summary:IndependenceSummary}>('/api/learning-os/transfer-challenge/evaluate',{method:'POST',body:data}),
}
